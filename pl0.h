...
long ll;               // line length
long kk;
long err;              // number of errors
long cx;               // code allocation index

char line[81];
char a[al+1];              // store current identifier
instruction code[cxmax+1]; //
char word[norw][al+1];     // names of reserved words
unsigned long wsym[norw];  // symbols of reserved words
unsigned long ssym[256];   // symbols of operations
...
struct{
    ...
    long level;
    long addr;
}table[txmax+1]; // name table: contains every description object of each process

char sourcefilename[80]; // name of source code file
FILE* sourcefile; // source code file
